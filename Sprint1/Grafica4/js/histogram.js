    ...
    init: function() {
        ...
        for(var i=0; i < datosAltura.length; i++) {
            ...
            elBox.addEventListener('mouseenter', function (event) {
                ...
                var cajaTexto = document.createElement('a-text');

                cajaTexto.setAttribute('value', "HOLA");
                cajaTexto.setAttribute('color', "#808000");
                //cajaTexto.setAttribute('align' , "center");
                cajaTexto.setAttribute('position', {
                    x: this.getAttribute('position').x,
                    y: this.getAttribute('position').y + this.getAttribute('height') + 8,
                    z: this.getAttribute('position').z + 4
                });



                console.log("Posicion Box: " + this.getAttribute('position').x);
                console.log("Altura Box: " + this.getAttribute('height'));
                //console.log("texto posicion: " + cajaTexto.getAttribute('position').x);

                entityEl.appendChild(cajaTexto);
